...
def fbrn(p, t, qv, u, v, hhl, hsurf):
    ...
    theta_surf_bc = np.broadcast_to(thetav[-1,:,:],hfl.shape) 
    nlevels_xr_bc = np.broadcast_to(np.expand_dims(np.expand_dims(nlevels_xr,-1),-1), hfl.shape)

    brn = pc_g * (hfl-np.broadcast_to(np.expand_dims(hsurf,0), hfl.shape)) * (thetav - theta_surf_bc) * nlevels_xr_bc /  \
        (np.broadcast_to(np.expand_dims(thetav_sum,0), hfl.shape) * (u_**2 + v_**2))
